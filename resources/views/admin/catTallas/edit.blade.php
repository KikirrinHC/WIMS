...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.catTalla.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.cat-tallas.update', [$catTalla->id]) }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipoprenda') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}" {{ (old('tipoprenda_id') ? old('tipoprenda_id') : $catTalla->
                        tipoprenda->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('talla') ? 'is-invalid' : '' }}" type="text" name="talla"
                    id="talla" value="{{ old('talla', $catTalla->talla) }}" required>
                @if($errors->has('talla'))
                <div class="invalid-feedback">
                    {{ $errors->first('talla') }}
                </div>
            ...
            <div class="form-group">
                ...
                <div class="form-check {{ $errors->has('estatus') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="radio" id="estatus_{{ $key }}" name="estatus"
                        value="{{ $key }}" {{ old('estatus', $catTalla->estatus) === (string) $key ? 'checked' : '' }}
                    required>
