    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.empleados.update", [$empleado->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('clave') ? 'is-invalid' : '' }}" type="text" name="clave" id="clave" value="{{ old('clave', $empleado->clave) }}" required>
                @if($errors->has('clave'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sucursal') ? 'is-invalid' : '' }}" name="sucursal_id" id="sucursal_id" required>
                    @foreach($sucursals as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sucursal_id') ? old('sucursal_id') : $empleado->sucursal->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
